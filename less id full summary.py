...
n_bootstraps = 200
kalman = False
output_pdf = 'kidney_less_id_summary_combined.pdf'
divisions = [1, 2, 4, 8, 16]

# Custom colors and markers for each division
div_colors = ['red', 'orange', 'forestgreen', 'blue', 'magenta']
div_markers = ['o', '^', 's', 'D', 'p']
...
for idx, (d, eigvals_matrix) in enumerate(all_eigvals):
    ...
    plt.plot(x, sorted_eigenvalues.real, marker=div_markers[idx], linestyle='None', 
             mfc='none', color=div_colors[idx], label=f'd={d} Baseline')

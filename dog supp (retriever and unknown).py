def preprocessing():
    ...
    # Step 1: Convert all breed entries to lowercase
    ov_df['breed'] = ov_df['breed'].str.lower()

    # Step 2: Standardize any entry containing 'unknown' to exactly 'unknown'
    ov_df['breed'] = ov_df['breed'].apply(lambda x: 'unknown' if 'unknown' in x else x)

    # Step 3: Count how many dogs of each (cleaned) breed
    breed_counts = ov_df['breed'].value_counts()

    print("🐕 Number of dogs per breed (with 'unknown' grouped):")
    print(breed_counts)
    ...
    ov_df['breed'] = ov_df['breed'].str.lower()
    ov_df['breed_status'] = ov_df['breed_status'].str.lower()

    # Initialize new column with NaNs
    ov_df['breed_type'] = np.nan

    # Set breed_type = 1 if breed contains 'retriever'
    ov_df.loc[ov_df['breed'].str.contains('retriever', na=False), 'breed_type'] = 1

    # Set breed_type = 0 if breed contains 'unknown' AND breed_status is 'mixed breed'
    condition = (ov_df['breed'].str.contains('unknown', na=False)) & (ov_df['breed_status'] == 'mixed breed')
    ov_df.loc[condition, 'breed_type'] = 0
    ...
    df = df.merge(ov_df[['dog_id', 'Sex', 'Fixed', 'breed_type']], on='dog_id', how='left')
    ...
    cols.remove('Age')
    cols.insert(len(df.columns) - 5, 'Age')
    ...
    df = df.dropna(subset=['breed_type'])
...
def prepare_model_data(df, kalman = False):
    ...
    x_cov = df[['Sex', 'Fixed', 'breed_type', 'Age_norm', 'ones']]
...
z_df, z_mu_df, biomarker_weights, sorted_eigenvalues, z_col_rank_map, mu_col_rank_map = get_z_variables(W, mu, df_valid, biomarker_columns, 'dog_eigenvalues')

...
n_bootstraps = 200
kalman = False
output_pdf = f'aug5_dog_bootstrap_({start_time}).pdf'
...
for i in tqdm(range(n_bootstraps), desc="Bootstrapping"):
    ...
    try:
        ...
        continue
all_drift_slopes.append(np.vstack(drift_slopes))
    ...
    with PdfPages(pdf_path) as pdf:
        for i, (z_col, mu_col) in enumerate(zip(z_col_rank_map, mu_col_rank_map)):
            ...
            for df_boot in z_mu_df_list:
                try:
                    df_b = df_boot[['Age', mu_col]].dropna().sort_values('Age').reset_index(drop=True)
                    if len(df_b) >= 2:
                        slope_j, intercept_j = np.polyfit(df_b['Age'], df_b[mu_col], deg=1)
                        yj = slope_j * x_pred + intercept_j
                        if yj.shape == x_pred.shape:  # Make sure shape matches
                            mu_boot_stack.append(yj)
                except Exception as e:
                    print(f"Skipping mu boot fit: {e}")
            ...
            print(f"[{z_col}] lowess std (first 5): {y_lowess_std[:5]}")
        ...
        plt.xticks(np.arange(1,len(eigvals0_sorted) + 1, 1))
        ...
        for idx, drift_matrix in enumerate(all_drift_slopes):
            mean_drifts = drift_matrix.mean(axis=0)
            std_drifts = drift_matrix.std(axis=0)

            plt.errorbar(x, mean_drifts, yerr=std_drifts, fmt='o', color='red', 
                        label='mu drift (bootstrap mean)', alpha=0.8, capsize=3)
        ...
        plt.xticks(np.arange(1,len(eigvals0_sorted) + 1, 1))
        plt.title('Steady Point Drift Rate Comparison')
        plt.legend()
        plt.tight_layout()
        pdf.savefig()
        plt.close()

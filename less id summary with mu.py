...
def linear_regression(data):
    ...
    for key in ['y_next', 'y_cur', 'x_cov']:
        data[key] = data[key].select_dtypes(include=[np.number])

    y_next = data['y_next'].select_dtypes(include=['number'])
    y_cur = data['y_cur'].select_dtypes(include=['number'])
    x_cov = data['x_cov'].select_dtypes(include=['number'])
...
def prepare_model_data(df, kalman=False, save_path=None):
    ...
    # Create 'ones' column for bias term
    df['ones'] = 1
    df['Age_norm'] = normalize(df, 'Age')
    df = df.sort_values(by=['Record ID', 'Age'], ascending=[True, True]).reset_index(drop=True)

    # Calculate delta_t
    delta_t = df.groupby('Record ID')['Age'].diff(periods=-1)
    delta_t = delta_t.astype(float)  # Ensure it's numeric

    # Compute shifted biomarker values
    y_cur = df[biomarker_columns]
    y_next = df.groupby('Record ID')[biomarker_columns].shift(-1)

    # Covariates
    x_cov = df[['Sex', 'DM Type', 'Age_norm', 'ones']]

    # Identify valid rows where y_next and delta_t are both defined
    valid_mask = (~y_next.isna().any(axis=1)) & (~delta_t.isna())
    ...
    return {
        ...
        'y_cur': y_cur[valid_mask],
        'x_cov': x_cov[valid_mask],
        'delta_t': -delta_t[valid_mask],  # Use negative dt
...
def plot_all_z_with_bootstrap(z_df0, z_mu_df0, sorted_eigenvalues, z_col_rank_map, mu_col_rank_map,
    ...
    base_ms = 10  # starting marker size
    base_lw = 2   # starting line width
    decay = 0.85  # shrink factor per layer

    for idx, (d, eigvals_matrix) in enumerate(all_eigvals):
        ms = base_ms * (decay ** idx)  # smaller for higher idx
        lw = base_lw * (decay ** idx)
        ...
        plt.plot(x, mean_eigs, linestyle='-', color=div_colors[idx], alpha=0.8, markersize=ms, linewidth=lw)
...
n_bootstraps = 200
kalman = False
output_pdf = 'kidney_less_id_summary_july_30.pdf'
...
with PdfPages(pdf_path) as pdf:
    for d in divisions:
        output_pdf_path = os.path.join('results/Kidney_Bootstrap', f'kidney_summary_july30_d{d}.pdf')
        ...
        with PdfPages(output_pdf_path) as pdf:
            ...
            for i in tqdm(range(n_bootstraps), desc=f"Bootstrapping d={d}"):
                np.random.seed(i)

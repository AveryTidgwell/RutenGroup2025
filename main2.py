...
def process_data(df, kalman = False):
    ...
    for biomarker in biomarker_columns:
        ...
        if num_removed > 0:
            removed = df.loc[mask, ['AnimalID', 'Age', biomarker]].copy()
    ...
    removed_df = pd.concat(removed_values) if removed_values else pd.DataFrame(columns=['AnimalID', 'Age', biomarker, 'biomarker'])

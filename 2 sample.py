...
def linear_regression(data):
    ...
    if y_next.shape[0] == 0 or y_cur.shape[0] == 0:
...
kalman = False
sample_limit = 2
output_pdf = '2_sample_cap_kidney.pdf'
...
for i in tqdm(range(n_bootstraps), desc="Capped samples per ID"):
    np.random.seed(i)

    # Group by ID and bootstrap up to sample_limit samples per person
    sampled_df_list = []
    for record_id, group in df.groupby('Record ID'):
        n_rows = len(group)
        if n_rows > sample_limit:
            sampled_rows = group.sample(n=sample_limit, replace=True, random_state=i)
        else:
            sampled_rows = group.copy()
        sampled_df_list.append(sampled_rows)

    df_capped = pd.concat(sampled_df_list).reset_index(drop=True)

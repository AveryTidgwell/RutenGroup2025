...
n_bootstraps = 200
kalman = False
output_pdf = 'z_bootstrap_kidney_july_22.2.pdf'
...
print("Running bootstraps by Record ID...")
...
for i in tqdm(range(n_bootstraps), desc="Bootstrapping"):
    ...
    boot_df_lengths.append(len(df_boot))  # Track number of rows
    ...
    try:
        ...
        continue
boot_df_lengths = np.array(boot_df_lengths)
print("\nBootstrap DataFrame Size Summary:")
print(f"  Min rows:  {boot_df_lengths.min()}")
print(f"  Max rows:  {boot_df_lengths.max()}")
print(f"  Mean rows: {boot_df_lengths.mean():.2f}")
...
def plot_all_z_with_bootstrap(z_df0, z_mu_df0, sorted_eigenvalues, z_col_rank_map, mu_col_rank_map,
    ...
    os.makedirs('results/Kidney_Bootstrap', exist_ok=True)
